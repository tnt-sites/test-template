...
/**
 * Every page, including the ones in subdirectories.
 *
 * A flat `readdirSync` sees only the top level, and this site keeps its doctor
 * and service pages in `about-us/` and `services/`. Those pages were therefore
 * invisible to the usage scan while their components were still counted as
 * convertible — so the deletion pass, which removes anything no *seen* page
 * points at, would have taken out ten components that nine real pages use.
 */
...
/**
 * Does a background value paint nothing?
 *
 * The literal cases are `none` / `transparent`. The one that matters in
 * practice is `background-color: var(--bg)`, which every migrated component
 * carries: the value is a *prop*, and on this site almost all of them resolve
 * to `transparent` because the frontmatter's fallback says so. Reading the
 * `var()` as paint refused all thirty components — including plain runs of
 * prose that paint nothing at all — so the variable is followed back to the
 * `… || "<value>"` default it is assigned from before the call is made.
 */
function isBlank(value, source) {
  const blank = (v) => /^(?:none|transparent|rgba\(0,\s*0,\s*0,\s*0\))$/i.test(v.trim());
  if (blank(value)) return true;

  const varName = value.match(/^var\(\s*(--[\w-]+)\s*\)$/)?.[1];
  if (!varName) return false;

  // `style={`--bg:${backgroundColorValue}`}` names the const feeding the var.
  const bound = source.match(new RegExp(`${varName}\\s*:\\s*\\$\\{(\\w+)\\}`))?.[1];
  if (!bound) return false;

  // `const backgroundColorValue = a || b || "transparent";`
...
function refuseReason(name, source) {
  ...
  // Explicit grid placement is a two-dimensional layout — the doctor pages'
  // mirrored portrait-beside-copy rows, rebuilt by hand after the capture
  // flattened them. `blocks` is a linear run and renders a floated figure, so
  // collapsing these would silently undo that layout. The `by wp-migrator`
  // marker does not catch them: refining a component in place leaves it there.
  if (/grid-template-columns|grid-column\s*:/.test(source)) return "has a placed grid layout";
  ...
  const paints = rootRules.some(([, body]) => {
    ...
    return !isBlank(decl[1].trim(), source);
  });
...
for (const file of listPages(PAGES)) {
