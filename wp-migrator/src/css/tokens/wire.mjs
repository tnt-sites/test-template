...
/**
 * Import the chrome token layer, and only that layer.
 *
 * `wireStyleImports` turns on the whole generated theme at once, which is right
 * when the operator has just reviewed a palette and a type ramp and wrong as a
 * side effect of a menu change: `dev-chrome` is the pass you re-run when the
 * footer gains a link, and it must not repaint the site to do it.
 *
 * Its own region, so the two can coexist — whichever of them wired an import
 * keeps ownership of it, and running both does not produce the file twice.
 */
export function wireChromeImport(writer, targetRoot, chromeFile) {
  // The entry point is found by walking out from the generated file rather
  // than assumed, since `--styles` may put it in `src/styles` or a level
  // deeper in `src/styles/source`.
  let stylesDir = path.dirname(chromeFile);
  while (!fs.existsSync(path.resolve(targetRoot, stylesDir, "style.pcss"))) {
    const up = path.dirname(stylesDir);
    if (up === stylesDir || up === ".") {
      return {
        ok: false,
        reason: `no style.pcss above ${chromeFile} — the chrome layer is generated but never imported`,
      };
    }
    stylesDir = up;
  }
  ...
  const rel = path.relative(path.resolve(targetRoot, stylesDir), path.resolve(targetRoot, chromeFile));
  const current = fs.readFileSync(abs, "utf8");

  // Last, so the source's measurements override the template's own variables.
  // The layer declares custom properties only, so importing it changes nothing
  // by itself — a component has to ask for a token before it has an effect.
  const next = upsertRegion(current, abs, "chrome-imports", `@import "${rel}";`);

  if (next !== current && !writer.dryRun) fs.writeFileSync(abs, next, "utf8");
  writer.results.push({
    path: entry,
    outcome: next === current ? "unchanged" : hasRegion(current, abs, "chrome-imports") ? "updated" : "created",
    detail: "region chrome-imports",
  });
