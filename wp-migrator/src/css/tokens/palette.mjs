  ...
  for (const rule of rules) {
    ...
    for (const decl of rule.decls) {
      ...
      for (const raw of colorsInValue(decl.value)) {
        ...
        if (alpha === 0) continue; // fully transparent carries no colour
        ...
        // Area is what the colour *contributes*, not the size of the box it is
        // painted in. `formatHex` throws the alpha away, so without this an
        // 11%-alpha decorative wash scores exactly like an opaque fill of the
        // same box — which is how a gold gradient overlay
        // (`#F7C45D1C`, used twice on one motion-effects layer) beat the blue
        // this site is actually built from and claimed `--color-brand`.
        //
        // Scaled rather than thresholded: a translucent colour is still a real
        // colour, and a large enough faint area can legitimately outrank a small
        // opaque one. It just should not do so at eight times its weight.
        entry.area += weight * alpha;
        entry.maxAlpha = Math.max(entry.maxAlpha ?? 0, alpha);
        entry.occurrences += 1;
...
export function clusterColors(weighted, { maxDeltaE = 0.025 } = {}) {
  ...
  for (const entry of sorted) {
    ...
    if (home) {
      ...
      // The cluster is as opaque as its most opaque member: near-duplicates
      // merge here, and a colour used opaquely once is a real colour even if
      // most of its other uses are washes.
      home.maxAlpha = Math.max(home.maxAlpha ?? 0, entry.maxAlpha ?? 1);
...
export function assignRoles(clusters, measured, { hoverColors = {}, baseColors = {} } = {}) {
  ...
  // A brand colour is one the site actually paints with. A colour that only
  // ever appears as a near-transparent wash is decoration: this site's gold
  // exists solely as `#F7C45D1C` (11% alpha) inside one gradient overlay, and
  // area-weighting alone still let it outrank the blue on some pages. Requiring
  // one substantially opaque use is what separates "the brand" from "a tint".
