  ...
  const render = (node, indent, itemVar) => {
    ...
    switch (node.kind) {
      ...
      case "button": {
        ...
        // Button marks its label as a CloudCannon text region with
        // data-prop="text" unless told otherwise, and that path resolves
        // against the section, where no `text` field exists. Point it at the
        // real prop, or turn editing off inside loops, which have no stable
        // path.
        if (!p) {
          emit(`${pad}<Button ${attrs} editable={false} text="${escapeAttr(node.text ?? "")}" link="${escapeAttr(node.attrs.href || "#")}" />`);
          return;
        }
        const textExpr = p.ref(p.name);
        const linkExpr = p.linkName ? p.ref(p.linkName) : '"#"';
        const editAttr = itemVar ? `editable={false}` : `data-prop="${p.name}"`;
        emit(`${pad}{${textExpr} && <Button ${attrs} ${editAttr} text={${textExpr}} link={${linkExpr} || "#"} />}`);
        return;
      }
      case "textlink": {
