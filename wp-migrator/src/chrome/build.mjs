...
export function buildNavData(extracted, current = {}, { iconSet = [] } = {}) {
  ...
  if (aside?.phone?.display) {
    ...
    const phoneIcon = aside.phone.icon && iconSet.length
      ...
      : null;
    const labelIcon = aside.icon && iconSet.length ? resolveIcon({ name: aside.icon }, iconSet) : null;
    next.topBar = {
      label: aside.label || "",
      ...(aside.link ? { link: rewriteHref(aside.link) } : {}),
      ...(labelIcon ? { icon: labelIcon.name } : {}),
      phone: {
   ...
   * placeholder ("Request an Appointment" → /request-an-appointment/). Where
   * the source has its own CTA, it wins — keeping the template's styling props
   * and replacing only what it says and where it goes.
   *
   * Where the source has none, the button goes. Left alone the placeholder is
   * the loudest thing in the migrated header and it is not the client's: a
   * button nobody designed, pointing at whichever page the migration happened
   * to create under that name. Substituting the site's own booking link
   * instead is no better — a header that never had a button is not improved by
   * being given one, and the difference shows immediately next to the
   * original. Sites that put their booking link only in the menu keep it
   * there.
   */
  const cta = extracted.header?.buttons?.[0];
  if (Array.isArray(next.pageButtons) && next.pageButtons.length) {
    next.pageButtons = cta?.text && cta.link
      ? [
  ...
  /*
   * The same for the template's secondary control — a search icon here. It is
   * a feature of the template rather than of the site, and a header that never
   * offered search does not start offering it because it was migrated. Cleared
   * rather than left alone, since the component reads an empty list as "no
   * button" and an absent one as "use the default".
   */
  if (!extracted.header?.buttons?.length && Array.isArray(next.buttonSections)) {
    next.buttonSections = [];
  }
  return next;
}
...
export function buildFooterData(extracted, current = {}, { iconSet = [] } = {}) {
  ...
  const contactBand = buildContactBand(extracted.footer?.embedBand);
  if (contactBand) next.contactBand = contactBand;
...
/**
 * The band of map and form the footer ends with.
 *
 * Both are third-party iframes, so what comes across is the URL and the height
 * the source gave it — enough for the target to draw the band in its own
 * markup, and the only form in which either survives a migration at all. The
 * map's URL matters most: it encodes the pin the practice placed, which is not
 * something a rebuild can guess from an address.
 *
 * The two headings are told apart by size rather than by tag, because a page
 * builder gives both the same one — the larger is the title and the smaller is
 * the line above it.
 */
function buildContactBand(band) {
  if (!band?.embeds?.length) return null;

  const map = band.embeds.find((e) => e.kind === "map");
  const form = band.embeds.find((e) => e.kind === "form") ?? band.embeds.find((e) => e.kind === "embed");
  if (!map && !form) return null;

  const headings = [...(band.headings ?? [])].sort((a, b) => b.size - a.size);
  const title = headings[0]?.text ?? "";
  const eyebrow = headings.find((h) => h.text !== title)?.text ?? "";

  return {
    ...(eyebrow ? { eyebrow } : {}),
    ...(title ? { title } : {}),
    ...(map ? { mapEmbedUrl: map.src, mapHeight: map.height ?? null } : {}),
    ...(form
      ? { formEmbedUrl: form.src, formHeight: form.height ?? null, formTitle: form.title || "" }
      : {}),
  };
}
...
function buildLinkColumns(columns, iconSet = [], siteName = "") {
  return (columns ?? [])
    ...
    .filter((column) => column.logo || isNavColumn(column));
}

const CONTACT_SCHEME_RE = /^(tel|sms|mailto):/i;
const SOCIAL_OR_MAP_RE =
  /(facebook|instagram|twitter|x\.com|linkedin|youtube|yelp|tiktok|pinterest|share\.google|g\.page|maps\.|\/maps|goo\.gl)/i;

/**
 * Is this a column of navigation links, or something else with a heading?
 *
 * Reading the footer by its headings finds every heading in it, and only some
 * of them head a list of links. The rest are the parts of the footer the
 * target already builds from other data or from its own components: the
 * contact column, whose phone and address come from `siteInfo.offices`; the
 * social row, which comes from `siteInfo.socials`; and the promotional bands
 * above — a heading, a paragraph, and one button — which are page sections
 * wearing a footer's clothes.
 *
 * Passing those through as link columns renders the address twice and turns
 * "Schedule Your Appointment at Our 5-Star Rated Dental Practice" into a
 * column title over a single link. What separates the real one is that it is
 * a list: two or more ordinary page links, none of which is a phone number, a
 * map, or a social profile.
 */
function isNavColumn(column) {
  const nav = (column.links ?? []).filter(
    (link) =>
      link.path &&
      link.path !== "#" &&
      !CONTACT_SCHEME_RE.test(link.path) &&
      !SOCIAL_OR_MAP_RE.test(link.path)
  );
  return nav.length >= 2;
}
