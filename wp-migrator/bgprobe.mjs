...
console.log(JSON.stringify(await p.evaluate(()=>{
  const o={};
  for (const c of ["definitions","call-out","faq-section","related-topics","additional-topics","related-links","inner-topbar","breadcrumbs","recent-posts"]) {
    const el=document.querySelector("."+c);
    if(!el){o[c]="—";continue;}
    const cs=getComputedStyle(el);
    o[c]={bg:cs.backgroundColor,color:cs.color,pad:cs.paddingTop,radius:cs.borderRadius};
  }
  return o;
}),null,1));
